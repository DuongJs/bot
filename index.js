...
if (nodeVersion.major < 12) {
    logger(`Cái Node.js ${process.version} không được hỗ trợ, vui lòng cài Node.js 13 để chạy được bot!`, "LỖI");
...
const dashboard = http.createServer(function (_req, res) {
    ...
    res.write("Chao Mung Tro Lai Van Thuan (RqzaX)");
    res.end();
});

dashboard.listen(process.env.port || 0);

logger("Opened server site...", "[ BẮT ĐẦU ]");
...
function startBot(message) {
    ...
    child.on("close",async (codeExit) => {
      var x = 'codeExit'.replace('codeExit',codeExit);
        if (codeExit == 1) return startBot("Đang Bắt Đầu...");
...
}).catch(err => logger("Unable to check update.", "[ kIỂM TRA CẬP NHẬT ]"));
